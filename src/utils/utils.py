...
def manual_designed_folder(result_path, params):
    '''
    manually designing this folder is for distinguish in different parameters
    '''
    table_path = result_path + params.dataset + "_mem=" + str(params.mem_size) + '/'
    table_path = table_path + str(params.learner)
    table_path = table_path + "_task=" + str(params.n_tasks)
    return table_path
...
def plt_img(combined_x_mix):
    ...
    # 计算总共需要多少行
    num_rows = -(-num_samples // num_samples_per_row)  # 向上取整

    # 创建子图
    fig, axs = plt.subplots(num_rows, num_samples_per_row, figsize=(16, num_rows * 4))  # 根据行数调整图像高度
    ...
    for i in range(num_samples):
        ...
        # 获取当前样本的图像数据
        sample_image = combined_x_mix[i].cpu().numpy().transpose(1, 2, 0)  # 将通道维度放在最后

        # 显示图像
        axs[row, col].imshow(sample_image)  # 使用默认的彩色颜色映射
