...
class MixUpAugment(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.max_mix = kwargs.get('max_mix', 0.5)

    def forward(self, batch1, batch2, label1, label2, **kwargs):
        coef = r.uniform(self.min_mix, self.max_mix)
        # n_concat = (len(batch1) // len(batch2)) + 1
        # batch2_ext = torch.cat([batch2[np.random.permutation(len(batch2)).tolist()] for _ in range(n_concat)],dim=0).to(device)
        # output = batch1.to(device) * coef + batch2_ext[:len(batch1)] * (1 - coef)

        # 计算需要扩展的倍数
        n_concat = (len(batch1) // len(batch2)) + 1
        # 扩展并随机排列 batch2 和 labels2
        indices = [np.random.permutation(len(batch2)).tolist() for _ in range(n_concat)]
        batch2_ext = torch.cat([batch2[i] for i in indices], dim=0).to(device)
        labels2_ext = torch.cat([label2[i] for i in indices], dim=0).to(device)
        # 按照混合系数进行混合
        mixed_batch = batch1.to(device) * coef + batch2_ext[:len(batch1)] * (1 - coef)
        mixed_labels = label1.to(device) * coef + labels2_ext[:len(label1)] * (1 - coef)
