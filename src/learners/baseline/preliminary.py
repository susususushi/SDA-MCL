        ...
        for j, batch in enumerate(dataloader):
            ...
            for _ in range(self.mem_iters):
                ...
                if mem_x.size(0) > 0:
                    ...
                    if self.params.augmentation == 'randaug1':
                        combined_x_aug = torch.cat((combined_x, combined_aug1))
                    elif self.params.augmentation == 'randaug2':
                        combined_x_aug = torch.cat((combined_x, combined_aug2))
                    elif self.params.augmentation == 'randaug3':
                        combined_x_aug = torch.cat((combined_x, combined_aug3))
                    elif self.params.augmentation == 'randaug4':
                        combined_x_aug = torch.cat((combined_x, combined_aug4))
                    elif self.params.augmentation == 'cutout':
                        combined_x_aug = torch.cat((combined_x, combined_augc))
                    elif self.params.augmentation == 'jigsaw':
                        combined_x_aug = torch.cat((combined_x, combined_augj))
                    elif self.params.augmentation == 'rotation':
                        combined_x_aug = torch.cat((combined_x, combined_augr))
                    combined_y_aug = torch.cat((combined_y, combined_y))
                    cs = combined_x.size(0)

                    # Inference
                    logits, feas = self.model.full(combined_x_aug)
        ...
        with torch.no_grad():
            ...
            with torch.autocast(device_type='cuda', dtype=torch.float16):
                for sample in dataloader:
                    ...
                    if self.params.augmentation == 'randaug1':
                        logits = self.model.logits(combined_x_aug1)
                    elif self.params.augmentation == 'randaug2':
                        logits = self.model.logits(combined_x_aug2)
                    elif self.params.augmentation == 'randaug3':
                        logits = self.model.logits(combined_x_aug3)
                    elif self.params.augmentation == 'randaug4':
                        logits = self.model.logits(combined_x_aug4)
                    elif self.params.augmentation== 'cutout':
                        logits = self.model.logits(combined_x_augc)
                    elif self.params.augmentation == 'jigsaw':
                        logits = self.model.logits(combined_x_augj)
                    elif self.params.augmentation == 'rotation':
                        logits = self.model.logits(combined_x_augr)
                    loss = nn.functional.cross_entropy(logits, labels.long(), reduction='none')
