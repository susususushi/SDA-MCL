...
class Parser:
    ...
    def __init__(self):
        ...
        parser.add_argument('--optim', default='SGD', choices=['Adam', 'SGD', 'AdamW'])
        ...
        parser.add_argument('--es-delta', type=float, help="minimum gap between each step for early stopping",
                            default=0.01)
        # Boosting params
        ...
        parser.add_argument('--eval-proj', action='store_true',
                            help="Use projection for inference. (default is representation.)")
        ...
        parser.add_argument('--nf', type=int, default=64,
                            help="Number of feature for Resnet18. Set nf=20 for reduced resnet18, nf=64 for full.")
        #####################
        # Dataset parameters
        #####################
        # parser.add_argument('--data-root-dir', default='/data/',
        #                     help='Root dir containing the dataset to train on.')
        parser.add_argument('--data-root-dir', default='/home/ssb/Project/dataset/',
                            help='Root dir containing the dataset to train on.')
        parser.add_argument('--min-crop', type=float, default=0.2,
                            help="Minimum size for cropping in data augmentation. range (0-1)")
        ...
        parser.add_argument('--num-workers', '-w', type=int, default=4, help='Number of workers to use for dataloader.')
        ...
        parser.add_argument("--labels-order", type=int, nargs='+',
                            help="In which order to you want to see the labels ? Random if not specified.")
        ...
        parser.add_argument('--spectral', action="store_true",
                            help="Use spectral clustering on representations as pseudo labels.")
        ...
        parser.add_argument('--p-before-gram', type=float, default=0,
                            help="Wait to have seen some data before using gram matrix.")
        parser.add_argument('--gram-crit', type=float, default=1,
                            help='Maximum value of gram matrix avg before using gram as pseudo label')
        parser.add_argument('--gram-mo', action='store_true', help="Computing gram matrix on memory only.")
        # Memory parameters
        parser.add_argument('--mem-size', type=int, default=200,
                            help='Memory size for continual learning')  # used also for ltm
        parser.add_argument('--mem-batch-size', '-mbs', type=int, default=200,
                            help="How many images do you want to retrieve from the memory/ltm")  # used also for ltm
        parser.add_argument('--buffer', default='reservoir',
                            help="What buffer do you want ? See available buffers in utils/name_match.py")
        parser.add_argument('--drop-method', default='random', choices=['random'],
                            help="How to drop images from memory when adding new ones.")
        # Pseudo labels parameters
        parser.add_argument('--classifier', choices=['ncm', 'knn'], default='ncm',
                            help='Classifier to use for pseudo labelli ng')
        ...
        parser.add_argument('--learner',
                            help='What learner do you want ? See list of available learners in utils/name_match.py')
        ...
        parser.add_argument('--lab-pc', type=int, default=20,
                            help="Number of labeled images per class to use in unsupervised evaluation.")
        # Multi runs arguments
        parser.add_argument('--n-runs', type=int, default=10, help="Number of runs, with different seeds each time.")
        ...
        parser.add_argument('--mem-iters', type=int, default=1,
                            help="Number of times to make a grad update on memory at each step")
        parser.add_argument('--review', '-rv', action='store_true', help="Review trick on last memory.")
        parser.add_argument('--rv-iters', '-rvi', type=int, default=20,
                            help="Number of iteration to do on last memory after training.")
        parser.add_argument('--tf-type', default="full", choices=['full', 'partial'],
                            help=' Data augmentation sequence to use.')
        ...
        parser.add_argument('--wandb-watch', action='store_true',
                            help="Watch the models gradient and parameters into wandb (can be slow).")
        ...
        # SDA-MCL
        parser.add_argument("--randaug-m", type=int, default=15)
        parser.add_argument("--randaug-n", type=int, default=3)
        parser.add_argument("--randaug2_weight", type=float, default=0.3, help="beta_ra")
        parser.add_argument("--rotation_weight", type=float, default=0.1, help="beta_rot")


        # Preliminary
        parser.add_argument('--augmentation', default="randaug1",
                            choices=['randaug1', 'randaug2', 'randaug3', 'randaug4', 'cutout', 'jigsaw', 'rotation', 'randaug2+rotation'])
        parser.add_argument("--jigsaw_n", type=int, default=2)
        parser.add_argument("--cutout_length", type=int, default=112) # cifar100-16,tiny-32,in100-112
        self.parser = parser
